 ...
 *
 * $Id: GarpNetworkReadyEvent.java,v 1.1 2005/02/22 03:44:26 stephen Exp $
 */
...
/**
 * Event type for GARP network ready events
 *
 * @author stephen Jul 23, 2003
 * @version 1.0
 *
 */
class GarpNetworkReadyEvent implements GarpEvent
{
  ...
  /**
   * Constructor for GarpNetworkReadyEvent.
   */
  public GarpNetworkReadyEvent(GUID[] newDevices, GUID[] goneDevices, GUID[] activeDevices, GUID[] nonactiveDevices)
  {
    // Save the parameters
  ...
  {
    return GarpNetworkReadyEventListener.class;
  }
  ...
  {
    // Check type
    if (!(listener instanceof GarpNetworkReadyEventListener))
    {
      throw new IllegalArgumentException("bad interface type");
    }

    // Forward
    ((GarpNetworkReadyEventListener)listener).networkReadyEvent(newDevices, goneDevices, activeDevices, nonactiveDevices);
  }
