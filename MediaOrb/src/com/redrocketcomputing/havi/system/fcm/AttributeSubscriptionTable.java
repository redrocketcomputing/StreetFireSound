 ...
 *
 * $Id: AttributeSubscriptionTable.java,v 1.1 2005/02/22 03:44:27 stephen Exp $
 */
...
class AttributeSubscriptionTable
{
  ...
  public synchronized void add(AttributeSubscription subscription)
  {
    ...
    Set indicatorSet = (Set)indicatorMap.get(new Short(subscription.getIndicator()));
    if (indicatorSet == null)
    {
      // Add it
      indicatorSet = new ListSet();
      indicatorMap.put(new Short(subscription.getIndicator()), indicatorSet);
    }
  ...
  public synchronized AttributeSubscription[] remove(SEID seid)
  {
    ...
    if (subscriptionSet != null)
    {
      ...
      for (int i = 0; i < subscriptions.length; i++)
      {
        ...
        Set indicatorSet = (Set)indicatorMap.get(new Short(subscriptions[i].getIndicator()));
        if (indicatorSet == null)
        {
          throw new IllegalStateException("AttributeSubscriptionTable is corrupted, missing indicator set");
        }
   ...
   */
  public synchronized AttributeSubscription[] getByIndicator(short indicator)
  {
    // Look up the seid subscription set and check state
    Set indicatorSet = (Set)indicatorMap.get(new Short(indicator));
    if (indicatorSet != null)
    {
      // Convert to array and return
