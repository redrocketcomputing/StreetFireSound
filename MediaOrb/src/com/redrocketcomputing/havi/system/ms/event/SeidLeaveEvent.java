 ...
 *
 * $Id: SeidLeaveEvent.java,v 1.1 2005/02/22 03:44:26 stephen Exp $
 */
 ...
 *
 */
public class SeidLeaveEvent implements Event
{
  private SEID seid;

  /**
   * Constructor for SeidLeaveEvent.
   * @param seid The seid which has left the system
   */
  public SeidLeaveEvent(SEID seid)
  {
    // Check parameter
    if (seid == null)
    {
  ...
  {
    return SeidLeaveEventListener.class;
  }
  ...
  {
    // Check type
    if (!(listener instanceof SeidLeaveEventListener))
    {
      throw new IllegalArgumentException("bad interface type");
    }

    // Forward
    ((SeidLeaveEventListener)listener).seidLeaveEvent(seid);
  }
