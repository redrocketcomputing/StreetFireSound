 ...
 *
 * $Id: BackupMaintenanceRequestHandler.java,v 1.1 2005/02/22 03:47:23 stephen Exp $
 */
 ...
 */
public class BackupMaintenanceRequestHandler implements MaintenanceRequestHandler
{
   ...
   */
  public BackupMaintenanceRequestHandler(DataInputStream inputStream, DataOutputStream outputStream)
  {
  ...
  public void executeRequest() throws MaintenanceException
  {
    try
    {
      ...
      LoggerSingleton.logInfo(this.getClass(), "executeRequest", "backing up database");

      // Launch backup process
      Process backupProcess = Runtime.getRuntime().exec("/sbin/backupdb");
      ...
      {
        // We failed
        throw new MaintenanceException("backup failed");
      }

      // Create file object for the backup file
      File backupFile = new File("/tmp/backup.rbx1600");

      // Create file sender
      FileSender fileSender = new FileSender(backupFile, outputStream);
