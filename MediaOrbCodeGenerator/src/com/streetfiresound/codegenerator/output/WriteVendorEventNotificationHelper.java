 ...
 *
 * $Id: WriteVendorEventNotificationHelper.java,v 1.2 2005/02/24 03:03:36 stephen Exp $
 */
 ...
 */
public class WriteVendorEventNotificationHelper implements ConstTypeDefinition
{
   ...
   */
  public WriteVendorEventNotificationHelper(FunctionType ft, HashMap constantMap) throws IOException
  {
    super();
    this.ft = ft;
	  ...
	  constantName = getConstantName(constantMap);
		if(constantName.length() == 0)
			throw new IOException("WriteVendorEventNotificationHelper error: constant name not found");


    String location = rootPath + "/" + packageName.replace('.','/') + "/" + system.replace('.','/');

	  if(checkLocation(location) == false)
			throw new IOException("WriteVendorEventNotificationHelper error: Directory not exist:" + location);
  ...
  private void outputToFile(OutputStream os) throws IOException
  {
	...
	private void printClass(OutputStream os) throws IOException
	{
		...
		os.write("EventNotificationHelper extends VendorEventNotificationHelper\n".getBytes());

	}
	private void printOpenClass(OutputStream os) throws IOException
	{
		os.write("\t{\n".getBytes());
    os.write("\t\tpublic final static short EVENT_BASE = ConstVendorEventType.".getBytes());
	...
	private void printConstructor1(OutputStream os) throws IOException
	{
		...
		os.write(className.getBytes());
		os.write("EventNotificationHelper(SoftwareElement softwareElement, OperationCode opCode, VendorId eventVendorId) throws HaviMsgException\n".getBytes());
  	os.write("\t\t{\n".getBytes());
  	os.write("\t\t\tsuper(softwareElement, opCode, ConstVendorEventType.".getBytes());
  	os.write(constantName.getBytes());
  	os.write(", eventVendorId);\n".getBytes());
  	os.write("\t\t}\n\n\n".getBytes());
	}
	...
	private void printConstructor2(OutputStream os) throws IOException
	{
  	os.write("\t\tpublic ".getBytes());
  	os.write(className.getBytes());
  	os.write("EventNotificationHelper(SoftwareElement softwareElement, OperationCode opCode, VendorId eventVendorId, ".getBytes());
  	os.write(className.getBytes());
	...
	private void printReceiveNotification(OutputStream os) throws IOException
	{
		...
		os.write("\t\tpublic boolean receiveVendorEventNotification(SEID posterSeid, VendorEventId vendorEventId, HaviByteArrayInputStream payload)\n".getBytes());
		os.write("\t\t{\n".getBytes());
