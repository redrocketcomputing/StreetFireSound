 ...
 *
 * $Id: WriteAsynResponseInvocationFactory.java,v 1.2 2005/02/24 03:03:36 stephen Exp $
 */
 ...
 */
public class WriteAsynResponseInvocationFactory
{
   ...
   */
  public WriteAsynResponseInvocationFactory(InterfaceType it) throws IOException
  {
    super();
    this.it = it;
...
//     String location = "/" + driver.BASEOUTPUTPATH.replace('.','/') + "/" + driver.packagePath.replace('.', '/');
     ...
     if(!newfile.exists())
      ...
      FileOutputStream fos = new GplHeaderFileOutputStream(location+"/"+CodeGenerator.currentPackage+ "AsyncResponseInvocationFactory.java");
 ...
 */
  private void printClass(OutputStream os) throws IOException
  {
    ...
    os.write("AsyncResponseInvocationFactory extends AsyncResponseInvocationFactory implements Const".getBytes());
    os.write(CodeGenerator.currentPackage.getBytes());
...
//      os.write(ft.getTypeName().getBytes());
      ...
      os.write("AsyncResponseInvocation.class;\n".getBytes());
