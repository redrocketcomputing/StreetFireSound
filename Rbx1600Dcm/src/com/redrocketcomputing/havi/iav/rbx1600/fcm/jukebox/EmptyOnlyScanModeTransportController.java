 ...
 *
 * $Id: EmptyOnlyScanModeTransportController.java,v 1.7 2005/03/16 04:25:03 stephen Exp $
 */
 ...
 */
public class EmptyOnlyScanModeTransportController extends AbstractScanModeTransportController
{

  public EmptyOnlyScanModeTransportController(SonyJukeboxFcm parent, Protocol protocol, PositionTracker positionTracker, TocBuilder tocBuilder, TransportStateTracker transportStateTracker, SlotCache slotCache)
  {
    super(ConstAvDiscScanMode.SCAN_EMPTY_ONLY, parent, protocol, positionTracker, tocBuilder, transportStateTracker, slotCache);
  }
  ...
  {
    return "EmptyOnlyScanModeTransportController";
  }
  ...
  public void run()
  {
    try
    {
      ...
      for (currentSlot = startSlot; currentSlot <= endSlot; currentSlot++)
      {
        ...
        // Only scan if slot cache thinks this slot is empty
        ItemIndex[] index = slotCache.getItemIndex(currentSlot);
        if (index[0].getTitle().equals("EMPTY"))
        {
          // Stop the player
    ...
    finally
    {
      ...
      // Update date parent
      parent.changeTransportState(STOPPED, ConstSonyJukeboxPlayMode.SCAN_EMPTY_ONLY);
    }
  }
}
